...
import {
  toggleCompleted, useAppDispatch, removeTask
} from '../../store';
...
const TaskItem: FC<TaskItemProps> = ({ id, title, completed, icon }) => {
  ...
  return !isEditMode ? (
    <div className="my-2 flex justify-between items-center transform motion-reduce:transform-none hover:-translate-x-1 hover:-translate-y-1 hover:scale-100 transition ease-in-out duration-300">
      <label
        className={
          `cursor-pointer flex items-center text-xl md:text-2xl`
        }
      >
        <input
          type="checkbox"
      ...
      <div className="inline-flex">
        <button
          className="hover:bg-secondary-500 hover:text-white text-secondary-500 font-bold py-2 px-4 rounded-l focus:outline-none focus:shadow-outline"
